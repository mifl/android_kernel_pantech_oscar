...
#if defined(T_OSCAR)
#define OEM_MAX17058_STD_FULL	(966)//	(1009)
#define OEM_MAX17058_STD_EMPTY	(5)//(-7)
#define OEM_MAX17058_EXT_FULL	(981)//	(1015)
...
/*
 * 20120404_khlee: 
 * battery status is a charging while either usb or ac is detected. 
 * except over, under temp...etc
 */
#define PANTECH_BATT_STATUS_CHANGE
...
#else /* CONFIG_PANTECH_BMS_UPDATE */
static void bms_notify(struct work_struct *work)
{
	struct bms_notify *n = container_of(work, struct bms_notify, work);
...
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
	pantech_pm_log(percent_soc, (unsigned int) chip->cable_type);
#endif
...
#endif /* FEATURE_ANDROID_PANTECH_USB_OTG_MODE || FEATURE_ANDROID_PANTECH_USB_SMB_OTG_MODE */
...
#endif /* PANTECH_CHARGER_TIME_LIMITATION */
...
static void unplug_check_worker(struct work_struct *work)
{
	...
	if (!chip->btm_isr_lock_work_is) {
		schedule_delayed_work(&chip->btm_isr_lock_work,
			round_jiffies_relative(msecs_to_jiffies
				(BTM_ISR_LOCK_WAIT_PERIOD_US)));
	...
	if (reg_loop & VIN_ACTIVE_BIT) {
		...
		if (ibat > 0) {
			...
			if (dc_chg_plugged_in && !usb_chg_plugged_in) {
				...
				schedule_delayed_work(&chip->unplug_wrkarnd_restore_work,
...
static irqreturn_t batt_removed_irq_handler(int irq, void *data)
{
	...
	if (status) {
		if (smem_id_vendor1_ptr->power_on_mode == CHG_PM_OFFLINE_NORMAL_BOOT_MODE) {
			...
			schedule_delayed_work(&chip->batt_remove_check_work,
				round_jiffies_relative(usecs_to_jiffies
					(BATT_REMOVE_CHECK_WAIT_PERIOD_US)));
#else
			msm_restart(0xC9,0);//arch_reset(0xC9, 0);			//20121120, djjeon, PMIC, WTR JB modify
...
#endif /* PANTECH_BATTERY_CHARING_DISCHARING_TEST || PANTECH_CHARGER_MONITOR_TEST */
...
#endif /* PANTECH_CHARGER_MONITOR_TEST */
...
#if defined(PANTECH_DSPS_MULTY_BATTERY_COVER)
